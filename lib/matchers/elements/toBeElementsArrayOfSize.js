...
function toBeElementsArrayOfSizeFn(received, expected, options = {}, driver) {
    ...
    return browserToUse.call(async () => {
        ...
        const pass = await utils_1.waitUntil(async () => {
            elements = await refetchElements_1.refetchElements(elements, numberOptions.wait, true);
            return utils_1.compareNumbers(elements.length, numberOptions);
        ...
        const message = utils_1.enhanceError(elements, error, arrLength, this, verb, expectation, '', numberOptions);
        return {
            pass,
            message: () => message
        };
